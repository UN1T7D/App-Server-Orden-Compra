<?php 
	...
	if ($contadorAux < $numeroEtapas) {		//SI ETAPA SIGUIENTE ES MENOR A EL NUMERO DE ETAPAS
			...
			if ($contadorAux == $contadorAux2) {
				...
				if ($prepapre4) {
					$alertProceso = "<div class='alert bg-success text-center'>
										Se actualizo correctamente la orden de compra <a href='workspace.php?funcion=exportacionesexpo' class='text-decoration-none text-white'>¡Actualice la pagina!<br><i style='border: none; background-color: transparent;'class='fas fa-sync-alt fa-'></i></a>
									</div>";
				} else {
		...
		if ($prepare4) {
			$alertProceso = "<div class='alert bg-success text-center'>
								Se actualizo correctamente la orden de compra a finalizada <a href='workspace.php?funcion=exportacionesexpo' class='text-decoration-none text-white'>¡Actualice la pagina!<br><i style='border: none; background-color: transparent;'class='fas fa-sync-alt fa-'></i></a>
							</div>";
		} else {
			$alertProceso = "<div class='alert bg-danger text-center'>
